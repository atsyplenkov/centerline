...
geos_ms_simplify <- function(geom, keep) {
  perimeter_length <- geos::geos_length(geom)

  point_count <- geom |> geos::geos_num_coordinates()

  point_density <- perimeter_length / point_count

  geos::geos_simplify(geom, tolerance = point_density / (keep * 7))
}

# Fast densification, similar behavior to {mapshaper} ms_simplify
geos_ms_densify <- function(geom, keep) {
  perimeter_length <- geos::geos_length(geom)
  ...
  geos::geos_densify(geom, tolerance = point_density / (keep))
}

geos_ksmooth <- function(input) {
  do.call(c, lapply(input, geos_ksmooth_master))
}

geos_ksmooth_master <- function(input) {
  check_package("smoothr")
  checkmate::assert_class(input, "geos_geometry")

  crs <- wk::wk_crs(input)

  num_coords <- geos::geos_num_coordinates(input)
  cent_length <- geos::geos_length(input)
...
reverse_lines_if_needed <- function(lines_list_geos, end_point) {
...
find_outer_nodes <- function(skeleton_geos) {
  ...
  lonely_start <- start_points[vapply(
    start_intersects,
    FUN = function(x) length(x) == 1,
    FUN.VALUE = logical(1),
    USE.NAMES = FALSE
  )]
  lonely_end <- end_points[vapply(
    end_intersects,
    FUN = function(x) length(x) == 1,
    FUN.VALUE = logical(1),
    USE.NAMES = FALSE
...
get_geos_ring <- function(geos_obj, i) {
  geos_obj |>
    geos::geos_ring_n(i) |>
...
list_geos_inner_rings <- function(geos_obj, num_rings) {
...
raybevel_to_geos <- function(rayskeleton, crs = NULL) {
