...
add_boundary_anchors <- function(skeleton, polygon, anchors) {
  ...
  # First implementation: use the same raw graph for analysis (single-graph).
  # If fixture-approved junction/scale assertions require dual-graph behavior,
  # fall back to a precision-cleaned analysis graph below.
  raw_graph <- skeleton_graph_build(ordinary_skeleton, crs = crs)
  raw_degree <- skeleton_graph_degree(raw_graph)
  raw_xy <- cbind(raw_graph$nodes$x, raw_graph$nodes$y)
  raw_node_pts <- skeleton_graph_node_points(raw_graph)

  # Precision-cleaned analysis graph for candidate discovery/scoring.
  # Raw graph remains the connector-target and postcondition authority.
  # Extreme scales fall back to the unrounded node set for analysis only.
  analysis_graph <- tryCatch(
    skeleton_graph_build(
      ordinary_skeleton,
      crs = crs,
      grid_size = grid_size
    ),
    error = function(e) raw_graph
  )

  analysis_degree <- skeleton_graph_degree(analysis_graph)
  analysis_xy <- cbind(analysis_graph$nodes$x, analysis_graph$nodes$y)
  analysis_node_pts <- skeleton_graph_node_points(analysis_graph)
  analysis_boundary_dist <- as.numeric(
    geos::geos_distance(analysis_node_pts, boundary)
  )

  analysis_junction_ids <- which(analysis_degree >= 3L)
  raw_junction_ids <- which(raw_degree >= 3L)

  if (length(analysis_junction_ids) == 0L || length(raw_junction_ids) == 0L) {
    stop(
  ...
  map_analysis_to_raw <- function(analysis_id) {
    if (identical(analysis_graph, raw_graph)) {
      return(as.integer(analysis_id))
    }
    cxy <- analysis_xy[analysis_id, 1:2, drop = FALSE]
    d <- sqrt(rowSums(
  ...
  mapped_raw <- vapply(
    analysis_junction_ids,
    map_analysis_to_raw,
    integer(1),
    USE.NAMES = FALSE
  )
  keep_map <- !is.na(mapped_raw)
  analysis_junction_ids <- analysis_junction_ids[keep_map]
  mapped_raw <- mapped_raw[keep_map]

  if (length(analysis_junction_ids) == 0L) {
    stop(
      "No valid interior junction connector; ",
  ...
  n_junctions <- length(analysis_junction_ids)
  # Junction coordinate matrix and raw targets (numeric hot path).
  j_xy <- analysis_xy[analysis_junction_ids, 1:2, drop = FALSE]
  j_raw_id <- as.integer(mapped_raw)

  # Adjacency once; avoids neighbors() per junction per anchor.
  analysis_adj <- vector("list", length(analysis_degree))
  for (cid in analysis_junction_ids) {
    analysis_adj[[cid]] <- skeleton_graph_neighbors(analysis_graph, cid)
  }

  select_inward_neighbor <- function(cleaned_id, anchor_xy) {
    neighbours <- analysis_adj[[cleaned_id]]
    if (length(neighbours) == 0L) {
      return(NA_integer_)
    }
    dists <- analysis_boundary_dist[neighbours]
    max_d <- max(dists)
    candidates <- neighbours[dists >= (max_d - grid_size)]
    if (length(candidates) == 1L) {
      return(candidates[[1]])
    }
    jxy <- analysis_xy[cleaned_id, 1:2]
    angles <- vapply(
      candidates,
      function(nid) {
        continuation_angle(anchor_xy, jxy, analysis_xy[nid, 1:2])
      },
      numeric(1),
      USE.NAMES = FALSE
    )
  ...
  score_junction_subset <- function(axy, j_idx) {
    ...
    for (i in seq_len(n)) {
      j <- j_idx[[i]]
      cid <- analysis_junction_ids[[j]]
      cxy <- j_xy[j, ]
      nid <- select_inward_neighbor(cid, axy)
      if (is.na(nid)) {
        next
      }
      ang <- continuation_angle(axy, cxy, analysis_xy[nid, 1:2])
      if (is.na(ang)) {
        next
      }
  ...
  already_terminal_on_skeleton <- function(anchor) {
    id <- skeleton_graph_nearest_nodes(raw_graph, anchor)
    if (
      as.numeric(geos::geos_distance(anchor, raw_node_pts[id])) != 0
    ) {
      return(FALSE)
    }
    if (raw_degree[[id]] != 1L) {
    ...
    neighbours <- skeleton_graph_neighbors(raw_graph, id)
  ...
  for (ai in seq_along(anchors)) {
    ...
    raw_near <- skeleton_graph_nearest_nodes(raw_graph, anchor)
    if (
      as.numeric(geos::geos_distance(anchor, raw_node_pts[raw_near])) == 0
    ) {
      stop(
  ...
  aug_graph <- skeleton_graph_build(augmented, crs = crs)
  aug_degree <- skeleton_graph_degree(aug_graph)
  aug_node_pts <- skeleton_graph_node_points(aug_graph)

  for (ai in seq_along(anchors)) {
    if (isTRUE(selected_meta[[ai]]$noop)) {
      next
    }
    zero_id <- skeleton_graph_nearest_nodes(aug_graph, anchors[ai])
    on_node <- as.numeric(
      geos::geos_distance(anchors[ai], aug_node_pts[zero_id])
    ) == 0
    if (!on_node || aug_degree[[zero_id]] != 1L) {
      stop(
