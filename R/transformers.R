...
terra_to_sf <- function(input) {
  spatial_data <- terra::as.data.frame(input)
...
terra_to_geos <- function(input) {
  input |> sf::st_as_sf() |> geos::as_geos_geometry()
  # input |>
  #   terra::geom(wkt = TRUE) |>
...
geos_to_terra <- function(input) {
  wk_input <- wk::as_wkt(input)

  terra::vect(as.character(wk_input), crs = wk::wk_crs(wk_input)$wkt)
}

# geos_geometry polygon to matrix of coordinates
geos_to_matrix <- function(geos_obj) {
