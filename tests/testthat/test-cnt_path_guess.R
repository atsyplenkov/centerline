...
test_that("cnt_path_guess inherits params", {
  ...
  result_straight_geos <- cnt_path_guess(
    polygon,
    method = "s",
    return_geos = TRUE
  )
  ...
  expect_false(identical(sf::st_length(result), sf::st_length(result_straight)))
...
test_that("cnt_path_guess works with 'sf' geometries", {
  polygon <- polygon_sf
  skeleton <- skeleton_sf
  ...
  expect_equal(as.numeric(round(sf::st_length(result), 5)), 50.95459)
  ...
  keep_list <- lapply(seq(0.1, 2, by = 0.2), function(x) {
    tryCatch(cnt_path_guess(polygon, keep = x), error = \(e) NA)
  })
  ...
  expect_equal(nrow(shapes_centerline), nrow(shapes))
  expect_identical(
  ...
  expect_equal(wk::wk_crs(multipolygon_centerline), wk::wk_crs(multipolygon))
  ...
  # Incorrect inputs
  expect_error(cnt_path_guess(input = polygon, skeleton = polygon))
  expect_error(cnt_path_guess(input = skeleton))
  expect_error(cnt_path_guess(skeleton = polygon))
})

test_that("cnt_path_guess works with 'sfc' geometries", {
  ...
  expect_contains(get_geom_type(result2), "LINESTRING")
  expect_equal(wk::wk_crs(result2), wk::wk_crs(polygon))
  expect_equal(sf::st_as_sf(result), sf::st_as_sf(result2))
  ...
  expect_contains(get_geom_type(shapes_centerline), "LINESTRING")
  expect_equal(wk::wk_crs(shapes_centerline), wk::wk_crs(shapes))
  expect_equal(length(shapes_centerline), length(shapes))
...
test_that("cnt_path_guess works with 'geos' geometries", {
  ...
  ## Check centerline lengths
  expect_length(result, 1)
  expect_equal(round(geos::geos_length(result), 5), 50.95459)
...
test_that("cnt_path_guess works with 'terra' geometries", {
  ...
  ## Check centerline lengths
  expect_length(result, 1)
  expect_equal(round(terra::perim(result), 5), 50.95459)
  ...
  expect_equal(terra::crs(multipolygon_centerline), terra::crs(multipolygon))
  expect_equal(nrow(multipolygon_centerline), 8L)
  expect_identical(
...
test_that("cnt_path_guess errors on incorrect input classes", {
  expect_error(cnt_path_guess(input = "polygon_sf"))
  expect_error(cnt_path_guess(input = 1))
  expect_error(cnt_path_guess(input = "polygon_sfc", skeleton = polygon_sf))
})
